...
describe('Calculate Command', () => {
  ...
  describe('verbose mode', () => {
    test('should show step-by-step calculation in verbose mode', async () => {
      ...
      expect(consoleMock.logs.some(log => log.includes('Expression'))).toBe(
        true
      );
  ...
  describe('error handling', () => {
    test('should handle invalid expressions', async () => {
      ...
      expect(consoleMock.errors.some(error => error.includes('Error'))).toBe(
        true
      );
    });
  ...
  describe('mathematical constants', () => {
    ...
    test('should handle custom constants', async () => {
      ...
      expect(consoleMock.logs.some(log => log.includes('299792458'))).toBe(
        true
      );
    });
  });
  ...
  describe('matrices and vectors', () => {
    test('should handle matrix operations', async () => {
      ...
      expect(
        consoleMock.logs.some(log => log.includes('[[6, 8], [10, 12]]'))
      ).toBe(true);
    });
  ...
  describe('input validation', () => {
    test('should validate expression length', async () => {
      ...
      expect(
        consoleMock.logs.length + consoleMock.errors.length
      ).toBeGreaterThan(0);
    });
