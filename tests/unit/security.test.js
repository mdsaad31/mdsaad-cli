...
describe('Security Services', () => {
  ...
  beforeEach(async () => {
    ...
    securityManager.keyFile = path.join(
  ...
  describe('Input Validator', () => {
    test('should validate email addresses correctly', () => {
      expect(inputValidator.validate('user@example.com', 'email')).toBe(
        'user@example.com'
      );
      expect(
        inputValidator.validate('test.email+tag@domain.co.uk', 'email')
      ).toBe('test.email+tag@domain.co.uk');

      expect(() => inputValidator.validate('invalid-email', 'email')).toThrow(
        'Invalid email format'
      );
      expect(() => inputValidator.validate('user@', 'email')).toThrow(
        'Invalid email format'
      );
    });

    test('should validate URL formats correctly', () => {
      expect(inputValidator.validate('https://example.com', 'url')).toBe(
        'https://example.com'
      );
      expect(
        inputValidator.validate('http://api.service.com/v1/data', 'url')
      ).toBe('http://api.service.com/v1/data');

      expect(() => inputValidator.validate('invalid-url', 'url')).toThrow(
        'Invalid URL format'
      );
      expect(() => inputValidator.validate('ftp://file.com', 'url')).toThrow(
        'Invalid URL format'
      );
    });

    test('should validate API keys correctly', () => {
      expect(inputValidator.validate('abc123_key-456', 'apiKey')).toBe(
        'abc123_key-456'
      );
      expect(inputValidator.validate('VALID_API_KEY_123', 'apiKey')).toBe(
        'VALID_API_KEY_123'
      );

      expect(() => inputValidator.validate('invalid@key!', 'apiKey')).toThrow(
        'API key contains invalid characters'
      );
      expect(() =>
        inputValidator.validate('key with spaces', 'apiKey')
      ).toThrow('API key contains invalid characters');
    });

    test('should validate mathematical expressions safely', () => {
      expect(inputValidator.validate('2 + 2 * 3', 'expression')).toBe(
        '2 + 2 * 3'
      );
      expect(inputValidator.validate('sin(pi/2) + cos(0)', 'expression')).toBe(
        'sin(pi/2) + cos(0)'
      );
      expect(inputValidator.validate('(10 + 5) / 3', 'expression')).toBe(
        '(10 + 5) / 3'
      );

      expect(() =>
        inputValidator.validate('eval(alert("xss"))', 'expression')
      ).toThrow('dangerous function');
      expect(() => inputValidator.validate('2 + 2)', 'expression')).toThrow(
    ...
    test('should validate city names correctly', () => {
      ...
      expect(inputValidator.validate('São Paulo', 'cityName')).toBe(
        'São Paulo'
      );

      expect(() => inputValidator.validate('City123', 'cityName')).toThrow(
        'invalid characters'
      );
      expect(() => inputValidator.validate('', 'cityName')).toThrow(
        'cannot be empty'
      );
    });
    ...
    test('should validate currency codes correctly', () => {
      ...
      expect(() => inputValidator.validate('usd', 'currencyCode')).toThrow(
        'Invalid currency code format'
      );
      expect(() => inputValidator.validate('USDD', 'currencyCode')).toThrow(
        'Invalid currency code format'
      );
    });
    ...
    test('should implement rate limiting', () => {
      ...
      expect(() => inputValidator.checkRateLimit(identifier, 5, 60000)).toThrow(
        'Rate limit exceeded'
      );
    });
    ...
    test('should handle length limits', () => {
      ...
      expect(() => inputValidator.validate(longInput, 'expression')).toThrow(
        'exceeds maximum length'
      );

      const validInput = 'a'.repeat(100);
      expect(inputValidator.validate(validInput, 'expression')).toBe(
        validInput
      );
    });
  });
  ...
  describe('Security Manager', () => {
    ...
    test('should handle password hashing and verification', () => {
      ...
      const isValid = securityManager.verifyPassword(
        password,
        hashed.hash,
        hashed.salt
      );
      ...
      const isInvalid = securityManager.verifyPassword(
        'wrongPassword',
        hashed.hash,
        hashed.salt
      );
    ...
    test('should implement login attempt rate limiting', () => {
      ...
      expect(() => securityManager.checkLoginAttempts(identifier)).toThrow(
  ...
  describe('Network Security', () => {
    ...
    test('should implement rate limiting for network requests', () => {
      ...
      expect(() => networkSecurity.checkRateLimit(hostname)).toThrow(
    ...
    test('should sign and verify requests', () => {
      ...
      const invalidVerification = networkSecurity.verifyResponse(
        { modified: 'data' },
        signature
      );
    ...
    test('should validate API response structure', () => {
      ...
      expect(
        networkSecurity.validateApiResponse(validResponse, expectedStructure)
      ).toBe(true);
      ...
      expect(() =>
        networkSecurity.validateApiResponse(invalidResponse, expectedStructure)
    ...
    test('should handle secure WebSocket configuration', () => {
      ...
      const secureOptions = networkSecurity.createSecureWebSocket(
        wsUrl,
        options
      );
      ...
      expect(() =>
        networkSecurity.createSecureWebSocket('ws://insecure.com')
  ...
  describe('Security Command Integration', () => {
    ...
    beforeEach(() => {
      ...
      command.securityManager.keyFile = path.join(
        global.testUtils.TEST_CONFIG_DIR,
        '.keys'
      );
    });
    ...
    test('should show security status', async () => {
      ...
      expect(result).toBe(true);
      expect(
        mockConsole.logs.some(log => log.includes('Security Status'))
      ).toBe(true);
    });
    ...
    test('should perform security audit', async () => {
      ...
      expect(result).toBe(true);
      expect(mockConsole.logs.some(log => log.includes('Security Audit'))).toBe(
        true
      );
    });
    ...
    test('should generate security report', async () => {
      ...
      expect(
        mockConsole.logs.some(log => log.includes('Security Report'))
      ).toBe(true);
    });

    test('should manage API keys', async () => {
      // Set API key
      const setResult = await command.setApiKey('testservice', {
        key: 'testkey123',
      });
      ...
      expect(mockConsole.logs.some(log => log.includes('testservice'))).toBe(
        true
      );
    ...
    test('should perform cleanup operations', async () => {
      ...
      expect(mockConsole.logs.some(log => log.includes('Cleaning up'))).toBe(
        true
      );
    });
    ...
    test('should handle command errors gracefully', async () => {
      ...
      command.securityManager.initialize = jest
        .fn()
      ...
      expect(
        mockConsole.errors.some(error =>
          error.includes('Security operation failed')
        )
      ).toBe(true);
    });
  });
