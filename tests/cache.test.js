...
describe('Cache Service', () => {
  ...
  beforeEach(async () => {
    ...
    testCacheDir = path.join(
      os.tmpdir(),
      `mdsaad-cache-test-${Date.now()}-${Math.random()}`
    );
  ...
  describe('Cache Middleware', () => {
    test('should serve from cache when available', async () => {
      const fetchFunction = jest
        .fn()
        .mockResolvedValue({ result: 'fresh data' });
    ...
    test('should handle fetch function errors', async () => {
      ...
      await expect(middleware('error-key', fetchFunction)).rejects.toThrow(
        'API Error'
      );
    });
  });
