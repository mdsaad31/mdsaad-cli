...
class OllamaService {
  ...
  async checkConnection() {
    try {
      ...
      throw new Error(
        `Ollama not accessible at ${this.baseUrl}: ${error.message}`
      );
    }
  }
  ...
  async generate(prompt, options = {}) {
    ...
    if (!this.hasModel(model)) {
      throw new Error(
        `Model ${model} not available. Available models: ${this.availableModels.map(m => m.name).join(', ')}`
      );
    }
  ...
  getDefaultModel() {
    ...
    const preferred = this.availableModels.find(
      model => model.name.includes('llama2') || model.name.includes('llama3')
    );
