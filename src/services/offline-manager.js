...
class OfflineManager {
  constructor() {
    ...
    this.cacheDirectory = path.join(os.homedir(), '.mdsaad-cli', 'offline-cache');
  ...
  async setupCacheDirectory() {
    try {
      ...
      const subdirs = ['weather', 'currency', 'translations', 'api-responses', 'assets'];
      for (const subdir of subdirs) {
        await fs.ensureDir(path.join(this.cacheDirectory, subdir));
      }
      
      debugService.debug('Cache directory setup completed', { path: this.cacheDirectory });
    } catch (error) {
  ...
  async loadCachedData() {
    try {
      const cacheFiles = await fs.readdir(this.cacheDirectory, { withFileTypes: true });
      ...
      debugService.debug('Failed to load cached data', { error: error.message });
    }
  }
  ...
  async checkNetworkStatus() {
    ...
    try {
      ...
      if (wasOnline !== this.isOnline) {
        debugService.debug(`Network status changed: ${this.isOnline ? 'online' : 'offline'}`);
      ...
      if (wasOnline) {
        debugService.debug('Network check failed, assuming offline', { error: error.message });
        await this.onNetworkDisconnected();
      }
    }
  }
  ...
  async prepareOfflineFallbacks() {
    try {
      ...
      for (const [feature, capability] of this.offlineCapabilities.entries()) {
        if (capability.canWorkOffline && !this.cachedData.has(capability.cacheKey)) {
          await this.createEmergencyCache(feature, capability);
        }
      }
    } catch (error) {
      debugService.debug('Failed to prepare offline fallbacks', { error: error.message });
    }
  }
  ...
  async createEmergencyCache(feature, capability) {
    try {
      ...
      switch (feature) {
        ...
        default:
          debugService.debug(`No emergency cache available for feature: ${feature}`);
          return;
      }
    ...
    return {
      commands: {
        ...
        convert: {
          ...
          examples: ['mdsaad convert 100 USD EUR', 'mdsaad convert 10 km miles']
        },
        help: {
  ...
  async cacheData(key, data, ttl = 3600000) { // Default 1 hour TTL
    try {
      ...
      debugService.debug(`Failed to cache data: ${key}`, { error: error.message });
      return false;
    }
  }
  ...
  async saveCacheToFile(key, data) {
    try {
      ...
      debugService.debug(`Failed to save cache file: ${key}`, { error: error.message });
    }
  }
  ...
  async executeWithOfflineFallback(feature, onlineOperation, options = {}) {
    ...
    if (this.isOnline && !options.forceOffline) {
      try {
        ...
        debugService.debug(`Online operation failed for ${feature}`, { error: error.message });
  ...
  async executeOfflineFallback(feature, capability, options = {}) {
    try {
      switch (capability.fallbackStrategy) {
        ...
        default:
          throw new Error(`Unknown fallback strategy: ${capability.fallbackStrategy}`);
      }
    } catch (error) {
      debugService.debug(`Offline fallback failed for ${feature}`, { error: error.message });
  ...
  getOfflineStatus() {
    return {
      ...
      capabilities: Object.fromEntries(
        Array.from(this.offlineCapabilities.entries()).map(([feature, capability]) => [
          feature,
          {
