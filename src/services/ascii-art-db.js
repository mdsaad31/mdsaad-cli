...
class AsciiArtDatabase {
  ...
  async initialize() {
    try {
      ...
      loggerService.info(
        `ASCII Art Database initialized with ${this.getTotalArtCount()} artworks`
      );
    } catch (error) {
  ...
  searchArt(query, options = {}) {
    ...
    const { category = null, limit = 10, fuzzy = true } = options;
