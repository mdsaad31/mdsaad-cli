...
class I18NService {
  ...
  async initialize() {
    try {
      ...
      if (
        systemLocale &&
        systemLocale !== this.fallbackLanguage &&
        this.isLanguageSupported(systemLocale)
      ) {
        try {
          ...
          console.warn(
            `Failed to load language ${systemLocale}, using English`
          );
        }
      }
    } catch (error) {
  ...
  async loadLanguage(language) {
    ...
    if (await fs.pathExists(translationFile)) {
      try {
        ...
        throw new Error(
          `Failed to parse translation file for ${language}: ${error.message}`
        );
      }
    } else {
  ...
  detectLocale() {
    ...
    for (const locale of sources) {
      if (locale) {
        ...
        if (
          familyMap[langCode] &&
          this.isLanguageSupported(familyMap[langCode])
        ) {
  ...
  translatePlural(key, count, params = {}) {
    ...
    const translation =
      this.getTranslation(pluralKey, this.currentLanguage) ||
      this.getTranslation(key, this.currentLanguage) ||
