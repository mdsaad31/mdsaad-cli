...
class SystemInfoPlugin {
  ...
  async initialize(api) {
    ...
    api.registerCommand('sysinfo', this.systemInfoCommand.bind(this), this.name);
  ...
  async uptimeCommand(args, options) {
    ...
    console.log(this.api.outputFormatter.success(`⏰ System Uptime: ${uptimeString}`));
    console.log(this.api.outputFormatter.info(`   Total seconds: ${uptime.toFixed(0)}`));
  ...
  async memoryCommand(args, options) {
    ...
    console.log(this.api.outputFormatter.formatObject('System Memory', systemMemory));
    ...
    console.log(this.api.outputFormatter.formatObject('Process Memory', processMemory));

    // Memory status indicator
    if (usedPercentage > 90) {
      console.log(this.api.outputFormatter.error('⚠️ High memory usage detected'));
    } else if (usedPercentage > 75) {
      console.log(this.api.outputFormatter.warning('⚠️ Moderate memory usage'));
    } else {
      console.log(this.api.outputFormatter.success('✅ Memory usage is normal'));
    }
