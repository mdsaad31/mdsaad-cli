...
class WeatherCommand {
    ...
    try {
      ...
      if (!keyStatus.weather) {
        if (useProxyAPI) {
          console.log(
            chalk.red(
              '❌ MDSAAD Weather Service is temporarily unavailable and no local API keys are configured.'
            )
          );
  ...
  async displayCurrentWeather(location, options = {}) {
    try {
      ...
      if (options.detailed) {
        ...
        console.log();
        console.log(
          outputFormatter.highlight(`${current.temperature}°`, 'temperature') +
        ...
        console.log(
          `${icon} ${outputFormatter.highlight(`${current.temperature}°`, 'temperature')} ` +
            `${chalk.gray('(')}${chalk[tempColor](`feels like ${current.feelsLike}°`)}${chalk.gray(')')}`
        );
        console.log(`   ${outputFormatter.info(current.condition)}`);
      }
      ...
      if (current.wind.speed > 0) {
        ...
        console.log(
          `   Speed: ${chalk.white(current.wind.speed)} ${this.getSpeedUnit(options.units || 'metric')} ${windDir ? `${windArrow} (${windDir})` : ''}`
        );
        if (current.wind.gust > 0) {
          console.log(
            `   Gusts: ${chalk.white(current.wind.gust)} ${this.getSpeedUnit(options.units || 'metric')}`
          );
        }
      }
      ...
      if (current.sunrise && current.sunset) {
        ...
        console.log(
          `   Sunrise: ${chalk.white(this.formatTime(current.sunrise))}`
        );
        console.log(
          `   Sunset:  ${chalk.white(this.formatTime(current.sunset))}`
        );
      }
      ...
      console.log(
        chalk.gray(
          `Data from ${weatherData.provider.toUpperCase()} • Last updated: ${this.formatTime(new Date(weatherData.timestamp))}`
        )
      );
    } catch (error) {
  ...
  async displayForecast(location, options = {}) {
    try {
      ...
      dailyForecasts.slice(0, days).forEach((day, index) => {
        ...
        console.log(
          chalk.white(`${dayName} ${chalk.gray(date.toLocaleDateString())}`)
        );
        ...
        console.log(
          `  ${icon} ${chalk[tempColor](`${day.temperature.max}°`)}${chalk.gray('/')}${chalk.blue(`${day.temperature.min}°`)} ${chalk.gray(day.condition)}`
        );

        if (day.pop > 0) {
          console.log(
            `     ${chalk.blue('💧')} ${day.pop}% chance of precipitation`
          );
        }
        ...
        if (day.wind && day.wind.speed > 0) {
          ...
          console.log(
            `     ${chalk.gray('💨')} ${day.wind.speed} ${this.getSpeedUnit(options.units || 'metric')} ${windDir || ''}`
          );
        }
      ...
      console.log(
        chalk.gray(
          `Data from ${forecastData.provider.toUpperCase()} • Last updated: ${this.formatTime(new Date(forecastData.timestamp))}`
        )
      );
    } catch (error) {
  ...
  async displayWeatherAlerts(location, options = {}) {
    try {
      ...
      alerts.forEach((alert, index) => {
        console.log(
          chalk.red(`${index + 1}. ${alert.event || 'Weather Alert'}`)
        );
        console.log(
          chalk.yellow(`   Severity: ${alert.severity || 'Unknown'}`)
        );

        if (alert.start) {
          console.log(
            `   Start: ${chalk.white(this.formatDateTime(new Date(alert.start * 1000)))}`
          );
        }

        if (alert.end) {
          console.log(
            `   End: ${chalk.white(this.formatDateTime(new Date(alert.end * 1000)))}`
          );
        }

        if (alert.description) {
          console.log(
            `   ${chalk.gray(alert.description.substring(0, 200))}${alert.description.length > 200 ? '...' : ''}`
          );
        }
  ...
  displayWeatherHeader(weatherData) {
    ...
    console.log(
      chalk.gray(`   ${location.lat.toFixed(4)}, ${location.lon.toFixed(4)}`)
    );
    console.log();
  }
  ...
  displayWeatherDetails(current, units) {
    ...
    if (current.uvIndex !== null && current.uvIndex !== undefined) {
      ...
      weatherData.push([
        'UV Index',
        `${current.uvIndex} ${this.getUvIndexLevel(current.uvIndex)}`,
      ]);
    }
    ...
    console.log(
      outputFormatter.formatTable('📊 Weather Details', weatherData, {
        compact: true,
        colors: {
          header: 'cyan',
          border: 'gray',
  ...
  displayAirQuality(airQuality) {
    ...
    if (airQuality.usEpaIndex) {
      const aqi = this.aqiLabels[airQuality.usEpaIndex] || {
        label: 'Unknown',
        color: 'gray',
      };
      const aqiIcon = this.getAirQualityIcon(airQuality.usEpaIndex);
      console.log(
        `   EPA Index:    ${aqiIcon} ${chalk[aqi.color](airQuality.usEpaIndex)} (${aqi.label})`
      );
    }

    if (airQuality.pm2_5) {
      console.log(
        `   PM2.5:        ${chalk.white(airQuality.pm2_5.toFixed(1))} μg/m³`
      );
    }

    if (airQuality.pm10) {
      console.log(
        `   PM10:         ${chalk.white(airQuality.pm10.toFixed(1))} μg/m³`
      );
    }
  }
  ...
  showServiceStatus() {
    ...
    console.log(
      `${chalk.cyan('Service Status:')} ${stats.isInitialized ? chalk.green('Initialized') : chalk.red('Not Initialized')}`
    );
    console.log(
      `${chalk.cyan('Active Providers:')} ${chalk.white(stats.activeProviders)}`
    );

    if (stats.providerNames.length > 0) {
      console.log(
        `${chalk.cyan('Provider Names:')} ${chalk.white(stats.providerNames.join(', '))}`
      );
    }

    console.log(
      `${chalk.cyan('Cache TTL:')} ${chalk.white(Math.round(stats.cacheTtl / 60000))} minutes`
    );
    console.log();
  ...
  showProviderStatus() {
    ...
    providers.forEach(provider => {
      ...
      console.log(
        `  API Key: ${keyStatus} ${provider.requiresApiKey ? '(Required)' : '(Optional)'}`
      );
      console.log();
    });
    ...
    if (providers.filter(p => p.isActive).length === 0) {
      console.log(
        chalk.yellow(
          '⚠️ No weather providers are active. Please configure API keys.'
        )
      );
      console.log(
        chalk.gray(
          'Use: mdsaad config set weather.openweathermap.apiKey <your-key>'
        )
      );
      console.log(
        chalk.gray(
          'Or:  mdsaad config set weather.weatherapi.apiKey <your-key>'
        )
      );
    }
  }
  ...
  showStatistics() {
    ...
    console.log(
      `${chalk.cyan('Initialization:')} ${stats.isInitialized ? 'Complete' : 'Pending'}`
    );
    console.log(
      `${chalk.cyan('Active Providers:')} ${stats.activeProviders}/${Object.keys(weatherService.providers).length}`
    );
    console.log(
      `${chalk.cyan('Cache Duration:')} ${Math.round(stats.cacheTtl / 60000)} minutes`
    );
    console.log();
    ...
    if (stats.activeProviders > 0) {
      ...
      console.log(
        chalk.red('No active providers. Weather functionality is limited.')
      );
    }
  }
  ...
  getWeatherIcon(conditionCode, iconCode) {
    ...
    if (conditionCode) {
      ...
      if (conditionCode >= 600 && conditionCode < 700)
        return this.weatherIcons.snow;
      if (conditionCode >= 700 && conditionCode < 800)
        return this.weatherIcons.mist;
    ...
    if (iconCode) {
      ...
      if (iconCode.includes('02') || iconCode.includes('03'))
        return this.weatherIcons.partlyCloudy;
  ...
  getWeatherAscii(conditionCode, iconCode) {
    if (conditionCode) {
      // OpenWeatherMap codes
      if (conditionCode >= 200 && conditionCode < 300)
        return weatherAscii.thunderstorm;
      if (conditionCode >= 300 && conditionCode < 400)
        return weatherAscii.rainy;
      if (conditionCode >= 500 && conditionCode < 600)
        return weatherAscii.rainy;
      if (conditionCode >= 600 && conditionCode < 700)
        return weatherAscii.snowy;
      if (conditionCode >= 700 && conditionCode < 800)
        return weatherAscii.foggy;
    ...
    if (iconCode) {
      ...
      if (iconCode.includes('02') || iconCode.includes('03'))
        return weatherAscii.partlyCloudy;
  ...
  showHelp() {
    ...
    console.log(
      '  mdsaad weather                     →  Current weather (auto-detected location)'
    );
    console.log(
      '  mdsaad weather "New York"          →  Current weather for specific city'
    );
    console.log(
      '  mdsaad weather "40.7128,-74.0060"  →  Weather for coordinates (lat,lon)'
    );
    console.log();
    ...
    console.log(
      '  -f, --forecast                     →  Show weather forecast'
    );
    console.log(
      '  --days <number>                    →  Forecast days (1-10, default: 5)'
    );
    console.log(
      '  -u, --units <system>               →  Temperature units (metric, imperial)'
    );
    console.log(
      '  --alerts                           →  Show weather alerts and warnings'
    );
    console.log();
    ...
    console.log(
      '  mdsaad weather stats               →  Show service statistics'
    );
    console.log();

    console.log(chalk.cyan('Examples:'));
    console.log(
      '  mdsaad weather                              # Auto-detect location'
    );
    console.log(
      '  mdsaad weather "London, UK" --detailed      # Detailed London weather'
    );
    console.log(
      '  mdsaad weather Tokyo --forecast --days 7    # 7-day Tokyo forecast'
    );
    console.log(
      '  mdsaad weather Miami --alerts               # Miami with alerts'
    );
    console.log(
      '  mdsaad weather "Paris" --units imperial     # Paris weather in Fahrenheit'
    );
    console.log();
  ...
  async handleProxyRequest(location, options) {
    try {
      ...
      if (result.success) {
        ...
        console.log(
          chalk.white(
            `🌡️ Temperature: ${data.current.temperature}°${options.units === 'imperial' ? 'F' : 'C'}`
          )
        );
        ...
        console.log(
          chalk.white(
            `🌬️ Wind: ${data.current.wind} ${options.units === 'imperial' ? 'mph' : 'kph'}`
          )
        );
        console.log(
          chalk.white(
            `👁️ Visibility: ${data.current.visibility} ${options.units === 'imperial' ? 'miles' : 'km'}`
          )
        );
        ...
        if (options.forecast && data.forecast) {
          ...
          data.forecast.forEach(day => {
            console.log(
              chalk.cyan(
                `${day.date}: ${day.min_temp}°-${day.max_temp}° ${day.condition}`
              )
            );
          });
        }
  ...
  displayFallbackWeather(fallbackData) {
    ...
    if (fallbackData && fallbackData.location) {
      ...
      console.log(
        outputFormatter.info('🌤️ Weather information temporarily unavailable')
      );
      console.log(
        outputFormatter.info('📊 Showing last known conditions or offline data')
      );
      ...
      if (fallbackData.condition) {
        console.log(
          outputFormatter.info(`☁️ Conditions: ${fallbackData.condition}`)
        );
      }
      ...
      console.log(
        outputFormatter.info('🌤️ Weather services are temporarily unavailable')
      );
      console.log(
  ...
  handleError(error) {
    ...
    if (error.message.includes('not found')) {
      console.log(
        chalk.yellow(
          '💡 Try using a more specific location name or coordinates'
        )
      );
      ...
      console.log(
        chalk.yellow('💡 Configure weather API keys to enable weather features')
      );
      console.log(
        chalk.gray('Use: mdsaad weather providers (to see current status)')
      );
      console.log(
        chalk.gray('Use: mdsaad weather help (for configuration instructions)')
      );
    } else if (
