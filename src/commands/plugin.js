  ...
  .action(async (options) => {
    try {
      ...
      if (!pluginManager.isInitialized) {
        console.log(outputFormatter.loading('🔌 Initializing plugin system...'));
...
async function listPlugins() {
  ...
  if (plugins.length === 0) {
    ...
    console.log(outputFormatter.info('Use "mdsaad plugin --create <name>" to create your first plugin'));
    return;
  }
  ...
  console.log(outputFormatter.formatTable('Plugins', pluginData, {
...
async function installPlugin(source) {
  try {
    console.log(outputFormatter.loading(`🔄 Installing plugin from: ${source}`));

    const plugin = await pluginManager.installPlugin(source);
    
    console.log(outputFormatter.success(`✅ Plugin installed successfully: ${plugin.name}`));
    ...
    if (plugin.description) {
      console.log(outputFormatter.info(`   Description: ${plugin.description}`));
    }
  } catch (error) {
    console.log(outputFormatter.error(`❌ Installation failed: ${error.message}`));
    console.log(outputFormatter.info('Use "mdsaad plugin --validate <path>" to check plugin format'));
  }
}
...
async function uninstallPlugin(pluginName) {
  try {
    ...
    console.log(outputFormatter.loading(`🔄 Uninstalling plugin: ${pluginName}`));
    
    await pluginManager.uninstallPlugin(pluginName);
    
    console.log(outputFormatter.success(`✅ Plugin uninstalled: ${pluginName}`));
  } catch (error) {
    console.log(outputFormatter.error(`❌ Uninstallation failed: ${error.message}`));
  }
}
...
async function enablePlugin(pluginName) {
  ...
  if (plugin.active) {
    console.log(outputFormatter.info(`ℹ️ Plugin already active: ${pluginName}`));
    return;
  }

  // For now, plugins are automatically enabled when loaded
  console.log(outputFormatter.info(`ℹ️ Plugin enable/disable functionality coming soon`));
  console.log(outputFormatter.muted('Plugins are currently auto-enabled when installed'));
}
...
async function disablePlugin(pluginName) {
  ...
  console.log(outputFormatter.info(`ℹ️ Plugin enable/disable functionality coming soon`));
  console.log(outputFormatter.info('Use "mdsaad plugin --uninstall" to remove plugins for now'));
}
...
async function showPluginInfo(pluginName) {
  ...
  if (plugin.packageJson) {
    ...
    const packageInfo = {
      ...
      'Keywords': plugin.packageJson.keywords ? plugin.packageJson.keywords.join(', ') : 'None'
    };
...
async function showPluginStats() {
  ...
  if (stats.totalPlugins > 0) {
    ...
    plugins.forEach(plugin => {
      ...
      console.log(`   ${status} ${plugin.name} (${plugin.type}) - ${plugin.commands.length} commands`);
    });
  }
}
...
async function createPluginTemplate(pluginName) {
  try {
    ...
    if (await fs.pathExists(pluginDir)) {
      console.log(outputFormatter.warning(`⚠️ Plugin directory already exists: ${pluginName}`));
      return;
    }

    console.log(outputFormatter.loading(`🔄 Creating plugin template: ${pluginName}`));
    ...
    const className = pluginName.split('-').map(part => 
...
`;
    ...
    console.log(outputFormatter.success(`✅ Plugin template created: ${pluginName}`));
    console.log(outputFormatter.info(`📁 Location: ${pluginDir}`));
    console.log(outputFormatter.info('Edit the files and install with: mdsaad plugin --install ' + pluginDir));
  } catch (error) {
    console.log(outputFormatter.error(`❌ Template creation failed: ${error.message}`));
  }
}
...
async function validatePlugin(pluginPath) {
  try {
    ...
    try {
      ...
      console.log(outputFormatter.error(`❌ Failed to load plugin: ${error.message}`));
      return;
    }
    ...
    if (isValid) {
      ...
      console.log(outputFormatter.muted('See plugin template: mdsaad plugin --create example'));
    }
  } catch (error) {
...
async function showPluginOverview() {
  ...
  const stats = pluginManager.getStatistics();
  console.log(outputFormatter.info(`📊 ${stats.totalPlugins} plugins installed (${stats.activePlugins} active)`));

  if (stats.totalPlugins > 0) {
    console.log('\n' + outputFormatter.subheader('Quick Actions'));
    console.log(outputFormatter.muted('   mdsaad plugin --list              List all plugins'));
    console.log(outputFormatter.muted('   mdsaad plugin --info <name>       Show plugin details'));
    console.log(outputFormatter.muted('   mdsaad plugin --stats             Show detailed statistics'));
  }

  console.log('\n' + outputFormatter.subheader('Plugin Development'));
  console.log(outputFormatter.muted('   mdsaad plugin --create <name>     Create plugin template'));
  console.log(outputFormatter.muted('   mdsaad plugin --validate <path>   Validate plugin format'));
  console.log(outputFormatter.muted('   mdsaad plugin --install <path>    Install plugin from file'));
