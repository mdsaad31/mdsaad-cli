...
class MaintenanceCommand {
  ...
  configure(commander) {
    const cmd = commander
      ...
      .option(
        '-c, --clean-cache [type]',
        'Clean cache files (all, expired, or number of days)'
      )
      .option('-t, --clean-temp', 'Clean temporary files')
      .option(
        '-l, --clean-logs [days]',
        'Clean log files older than specified days (default: 30)'
      )
        ...
        try {
          ...
          console.error(
            outputFormatter.error(
              `Maintenance command failed: ${error.message}`
            )
          );
          debugService.debug('Maintenance command error', {
            error: error.message,
            stack: error.stack,
          });
          process.exit(1);
    ...
    try {
      ...
      console.log(
        outputFormatter.createTable([
          ...
          [
            'Total Memory',
            maintenanceService.formatBytes(diagnostics.system.totalMemory),
          ],
          [
            'Free Memory',
            maintenanceService.formatBytes(diagnostics.system.freeMemory),
          ],
      ...
      console.log(
        outputFormatter.createTable([
          ...
          [
            'Config Exists',
            diagnostics.mdsaad.configExists ? '✅ Yes' : '❌ No',
          ],
          ['Cache Exists', diagnostics.mdsaad.cacheExists ? '✅ Yes' : '❌ No'],
          [
            'Plugins Directory',
            diagnostics.mdsaad.pluginsExists ? '✅ Yes' : '❌ No',
          ],
        ])
      );
      ...
      const configStatus = diagnostics.configuration.valid
        ? '✅ Valid'
        : '❌ Invalid';
      console.log(
        outputFormatter.createTable([
      ...
      console.log(
        outputFormatter.createTable([
          ...
          [
            'Total Size',
            maintenanceService.formatBytes(diagnostics.cache.size),
          ],
      ...
      const networkStatus = diagnostics.network.connected
        ? '✅ Connected'
        : '❌ Offline';
      console.log(
        outputFormatter.createTable([
          ...
          [
            'WeatherAPI',
            diagnostics.network.apiReachable.WeatherAPI
              ? '✅ Reachable'
              : '❌ Unreachable',
          ],
          [
            'ExchangeRate-API',
            diagnostics.network.apiReachable['ExchangeRate-API']
              ? '✅ Reachable'
              : '❌ Unreachable',
          ],
          [
            'npm Registry',
            diagnostics.network.apiReachable['npm Registry']
              ? '✅ Reachable'
              : '❌ Unreachable',
          ],
        ])
      ...
      console.log(
        outputFormatter.createTable([
          [
            'Config Readable',
            diagnostics.permissions.configDirReadable ? '✅ Yes' : '❌ No',
          ],
          [
            'Config Writable',
            diagnostics.permissions.configDirWritable ? '✅ Yes' : '❌ No',
          ],
          [
            'Temp Writable',
            diagnostics.permissions.tempDirWritable ? '✅ Yes' : '❌ No',
          ],
        ])
      );
      ...
      console.log(
        outputFormatter.info(
          `Overall Status: ${healthIcon} ${diagnostics.health.overall.toUpperCase()}`
        )
      );
      ...
      spinner.stop();
      console.log(
        outputFormatter.error(`❌ Diagnostics failed: ${error.message}`)
      );
  ...
  async cleanCache(type) {
    ...
    try {
      ...
      spinner.stop();
      console.log(
        outputFormatter.error(`❌ Cache cleanup failed: ${error.message}`)
      );
    }
  }
  ...
  async cleanTemp() {
    ...
    try {
      ...
      console.log(
        outputFormatter.error(
          `❌ Temporary files cleanup failed: ${error.message}`
        )
      );
    }
  }
  ...
  async cleanLogs(maxAgeDays) {
    ...
    const spinner = outputFormatter.spinner(
      `Cleaning logs older than ${maxAgeDays} days...`
    );
    spinner.start();
    ...
    try {
      ...
      spinner.stop();
      console.log(
        outputFormatter.error(`❌ Log cleanup failed: ${error.message}`)
      );
    }
  }
  ...
  async migrateConfig() {
    ...
    try {
      ...
      if (result.success) {
        if (result.migrated) {
          console.log(
            outputFormatter.success('✅ Configuration migrated successfully')
          );
        } else {
          console.log(
            outputFormatter.info('ℹ️  Configuration is already up to date')
          );
        }
      } else {
        console.log(
          outputFormatter.error(
            `❌ Configuration migration failed: ${result.error}`
          )
        );
      }
    } catch (error) {
      ...
      console.log(
        outputFormatter.error(
          `❌ Configuration migration failed: ${error.message}`
        )
      );
    }
  }
  ...
  async quickHealth() {
    ...
    try {
      ...
      spinner.stop();
      console.log(
        outputFormatter.error(`❌ Health check failed: ${error.message}`)
      );
    }
  }
  ...
  async autoFix() {
    ...
    try {
      ...
      spinner.stop();
      console.log(
        outputFormatter.error(`❌ Auto fix failed: ${error.message}`)
      );
    }
  }
  ...
  async showMaintenanceMenu() {
    ...
    console.log(
      outputFormatter.info(
        '  mdsaad maintenance --diagnostics     Run full system diagnostics'
      )
    );
    console.log(
      outputFormatter.info(
    ...
    console.log(
      outputFormatter.info(
        '  mdsaad maintenance --clean-cache     Clean all cache files'
      )
    );
    console.log(
      outputFormatter.info(
        '  mdsaad maintenance --clean-cache expired  Clean expired cache only'
      )
    );
    console.log(
      outputFormatter.info(
    ...
    console.log(
      outputFormatter.info(
        '  mdsaad maintenance --clean-logs      Clean old log files'
      )
    );
    ...
    console.log(
      outputFormatter.info(
        '  mdsaad maintenance --storage         Show storage usage'
      )
    );
    ...
    console.log(
      outputFormatter.info(
        '  mdsaad maintenance --migrate-config  Migrate configuration'
      )
    );
    console.log(
      outputFormatter.info(
    ...
    console.log(
      outputFormatter.info('  mdsaad maintenance --reset [type]    Reset data')
    );
  }
